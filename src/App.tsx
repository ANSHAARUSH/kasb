...
const KasbReview = lazyWithRetry(() => import("./pages/dashboard/KasbReview"))
...
function App() {
  ...
  const [autoFillUrl, setAutoFillUrl] = useState<string | null>(null);

  useEffect(() => {
    const handleShowVault = (e: Event) => {
      const customEvent = e as CustomEvent<{ url: string }>;
      setAutoFillUrl(customEvent.detail.url);
    };
    window.addEventListener('show-autofill-vault', handleShowVault);
    return () => window.removeEventListener('show-autofill-vault', handleShowVault);
  }, []);
        ...
        {showSplash ? (
          ...
          <AuthProvider key="auth-provider">
            <ToastProvider>
              <ChatProvider>
                <Router>
                  ...
                  <Route path="/dashboard" element={<DashboardLayout />}>
                    ...
                    <Route path="startup/review" element={<KasbReview />} />
              ...
              <AutoFillModal 
                  isOpen={!!autoFillUrl} 
                  onClose={() => setAutoFillUrl(null)} 
                  onProceed={(url) => {
                      // Do NOT close the vault here! The user needs it open to copy-paste into the new tab.
                      window.open(url, '_blank', 'noopener,noreferrer');
                  }}
                  targetUrl={autoFillUrl || ''} 
              />
            </Router>
          </ChatProvider>
        </ToastProvider>
