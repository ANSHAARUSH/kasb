...
// ─── Full Composite Scoring (AI + Taste + Plan + Profile) ───────────────────

/**
 * Full investor score for startup feed.
 *
 * Weights:
 *   65% AI semantic match (from aiMatchingEngine)
 *   20% Subscription plan (startup + investor tiers)
 *   15% Profile completeness (startup + investor)
 *
 * Taste boost (additive, up to +15 pts on final score):
 *   If user has clicked similar investors before, boost this one.
 *
 * Falls back to static scoreInvestorForStartup() if AI score not available.
 */
export function scoreInvestorFull(
    investor: Investor,
    aiScore: number | null,              // null = AI not available for this investor
    tasteScore: number,                  // 0-100 from userTasteTracker
    startupTier: string,
    startupCompletion: number,
    // Legacy static params (used as fallback)
    startupTags: string[],
    startupIndustry: string,
    startupState?: string,
    startupCity?: string
    ...
    if (aiScore !== null && aiScore > 0) {
        ...
        base = aiScore * 0.65 + planScore * 0.20 + profileScore * 0.15
    } else {
        // Static fallback
        const staticResult = scoreInvestorForStartup(investor, startupTags, startupIndustry, startupState, startupCity)
        base = staticResult.total
    }

    // Taste boost: if user has clicked this type of investor before, add up to +15 pts
    const tasteBoost = tasteScore * 0.15

    const total = Math.min(100, Math.round(base + tasteBoost))
    ...
    return {
        ...
        breakdown: {
            ai: aiScore ?? 0,
            taste: tasteBoost,
            plan: aiScore !== null ? combinedPlanScore(startupTier, investor.tier || 'explore') : 0,
            profile: aiScore !== null ? combinedProfileScore(startupCompletion, investor.completionPercentage || 0) : 0,
        }
    }
}

/**
 * Full startup score for investor feed.
 *
 * Uses the static algorithm for the base score since investors don't get
 * AI-generated startup scores (only startups get AI investor scoring).
 * Taste score is additive boost based on what startup types the investor
 * has been engaging with.
 */
export function scoreStartupFull(
    startup: Startup,
    tasteScore: number,       // 0-100 from userTasteTracker
    investorExpertise: string[],
    investorIndustryFocus: string[],
    ...
    const staticResult = scoreStartupForInvestor(startup, investorExpertise, investorIndustryFocus, investorState, investorCity)

    // Taste boost: if investor keeps clicking similar startups, boost this one
    const tasteBoost = tasteScore * 0.15
    const total = Math.min(100, Math.round(staticResult.total + tasteBoost))

    return {
        total,
        breakdown: {
            ...staticResult.breakdown,
            taste: tasteBoost
        }
    }
}

// Internal helper — reused in scoreInvestorFull
function combinedPlanScore(startupTier: string, investorTier: string): number {
    ...
    return Math.round((startupScore + investorScore) / 2)
}
