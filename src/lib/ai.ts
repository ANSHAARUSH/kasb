...
export interface DynamicDocumentReview {
...
- Still be logical and accurate — brutal doesn't mean wrong. It means painfully honest.`;
    ...
    // Inject user's startup profile context so the mentor already knows their startup
    if (startupContext && startupContext.trim().length > 0) {
        systemPrompt += `\n\n---\nUSER'S STARTUP CONTEXT (This user has already provided their startup details on the platform. You already know this information — do NOT ask them to re-introduce their startup. Reference these details naturally and proactively in your advice when relevant):\n${startupContext}\n---`;
    }
