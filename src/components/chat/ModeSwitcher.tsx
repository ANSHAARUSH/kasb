...
export const ModeSwitcher: React.FC<ModeSwitcherProps> = ({ activeMode, onModeChange, isHeader }) => {
    if (isHeader) {
        return (
            <div className={cn(
                "flex flex-col p-1 rounded-2xl border transition-all duration-300 shadow-lg gap-1",
                activeMode === 'piranhatank' 
                    ? "bg-[#0c0c0c] border-red-900/40 shadow-red-900/10" 
                    : "bg-white border-gray-100 shadow-sm"
            )}>
                <button 
                    onClick={() => onModeChange('foundergpt')}
                    className={cn(
                        "flex items-center justify-center gap-2 py-2 px-3 rounded-xl text-[9px] font-black uppercase tracking-wider transition-all duration-300",
