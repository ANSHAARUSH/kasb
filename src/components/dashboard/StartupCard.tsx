...
import { BookmarkPlus, MessageSquare, UserPlus, Clock, CheckCircle, Briefcase, Globe } from "lucide-react"
...
export function StartupCard({ startup, onClick, onDoubleClick, isSelected, isSaved = false, onToggleSave, onMessageClick, triggerUpdate, onConnectionChange, showImpactPoints }: StartupCardProps) {
    ...
    const handleConnect = async (e: React.MouseEvent) => {
        ...
        // Allow peer-to-peer startup connections even on free plans
        const isPeerConnection = role === 'startup'
        if (!isPeerConnection && !subscriptionManager.canContact(startup.id)) {
    ...
    return (
        ...
        >
            ...
            <CardContent className="p-4 flex-1 flex flex-col gap-3">
                ...
                {/* Middle Row: Problem Statement (Matching Investor Bio Style) */}
                ...
                {/* Tags, Industry & Location (Subtle Footer) */}
                <div className="flex flex-wrap items-center gap-1.5 mt-auto pt-1">
                    {startup.industry && (
                        <span className="text-[9px] font-black text-indigo-600/60 uppercase tracking-tighter border-r pr-1.5 border-black/10">
                            {startup.industry}
                        </span>
                    )}
                    ...
                    {(startup.city || startup.state) && (
                        <span className="ml-auto text-[9px] font-bold text-gray-400 uppercase tracking-tight flex items-center gap-1">
                             <Globe className="h-2 w-2" /> {startup.city || startup.state}
                        </span>
                    )}
                </div>
            </CardContent>
