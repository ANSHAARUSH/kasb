...
import { LiquidRedirectBar } from "../common/LiquidRedirectBar"
    ...
    return (
        <div className="h-full flex flex-col bg-white overflow-hidden">
            ...
            <div className="flex-none px-6 pt-6 pb-4 border-b border-gray-50">
                <div className="flex items-start justify-between mt-2">
                    ...
                    <div className="flex items-center gap-1">
                        {onResize && (
                            <>
                                <Button variant="ghost" size="icon" onClick={() => { console.log('[InvestorDetail] Minimize clicked'); onResize('minimized'); }} className="rounded-full hover:bg-gray-100 hidden lg:flex">
                                    <Minus className="h-4 w-4" />
                                </Button>
                                {currentSize === 'full' ? (
                                    <Button variant="ghost" size="icon" onClick={() => { console.log('[InvestorDetail] Restore clicked'); onResize('default'); }} className="rounded-full hover:bg-gray-100 hidden lg:flex">
                                        <Minimize2 className="h-4 w-4" />
                                    </Button>
                                ) : (
                                    <Button variant="ghost" size="icon" onClick={() => { console.log('[InvestorDetail] MAXIMIZE clicked, currentSize=', currentSize); onResize('full'); }} className="rounded-full hover:bg-gray-100 hidden lg:flex">
                    ...
                    <Button
                        ...
                        onClick={() => {
                            if (investor.website) {
                                ...
                                const isEnabled = localStorage.getItem('kasb_autofill_enabled') !== 'false';
                                if (user && isEnabled) {
                                    window.dispatchEvent(new CustomEvent('show-autofill-vault', { detail: { url } }));
                                } else {
                                    setPendingRedirectUrl(url);
                                }
                            } else {
                                alert('No website provided for this firm.');
                            }
                        }}
            ...
            <LiquidRedirectBar 
                ...
                onComplete={() => setPendingRedirectUrl(null)} 
            />
        </div>
    )
}
