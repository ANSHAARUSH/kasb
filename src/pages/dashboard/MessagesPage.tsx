...
export default function MessagesPage() {
    ...
    const processConversations = useCallback(async (msgs: Message[]) => {
        ...
        // Important: Add all admin IDs to the "accepted" set so they are always visible
        adminData?.forEach((a: any) => acceptedIds.add(a.id))
        ...
        const profiles = [
            ...
            ...Array.from(adminIds).map((id: any) => ({ id, name: 'Kasb.AI', avatar: `${import.meta.env.BASE_URL}logo.webp` }))
        ]
        ...
        // 3. Ensure official Kasb.AI Admin chat is present even if no messages yet
        // Only if the user isn't an admin themselves
        const { data: isUserAdmin } = await supabase.from('admins').select('id').eq('id', user.id).single()
        if (!isUserAdmin && adminData && adminData.length > 0) {
            const primaryAdminId = adminData[0].id
            const hasAdminChat = finalConversations.some(c => c.userId === primaryAdminId)
            
            if (!hasAdminChat) {
                const officialChat: Conversation = {
                    userId: primaryAdminId,
                    name: 'Kasb.AI',
                    avatar: `${import.meta.env.BASE_URL}logo.webp`,
                    lastMessage: 'Official Channel',
                    time: '',
                    unread: 0
                }
                // Place it right after AI assistant or at the top
                finalConversations.splice(subscriptionManager.canUseAIChat() ? 1 : 0, 0, officialChat)
            }
        }
