...
import { reviewDocumentDynamic } from "../../lib/ai";
...
function ScorecardResult({ scorecard, theme }: { scorecard: any, theme: any }) {
    const defaultColors = ['bg-blue-500', 'bg-purple-500', 'bg-green-500', 'bg-orange-500', 'bg-pink-500', 'bg-indigo-500'];

    // Ensure backwards compatibility if old session format is loaded
    const isDynamic = Array.isArray(scorecard.categories);
    const categoriesToRender = isDynamic 
        ...
        : [
            { label: 'Market Opportunity', score: scorecard.scores?.market_opportunity, color: 'bg-blue-500' },
            { label: 'Product & Solution', score: scorecard.scores?.product_solution, color: 'bg-purple-500' },
            { label: 'Business Model', score: scorecard.scores?.business_model, color: 'bg-green-500' },
            { label: 'Team & Execution', score: scorecard.scores?.team, color: 'bg-orange-500' },
            { label: 'Financials & Ask', score: scorecard.scores?.financials, color: 'bg-pink-500' }
        ];
                    ...
                    <div className="flex items-center justify-between mb-8 gap-4">
                        <div>
                            <h2 className="text-2xl sm:text-3xl font-black tracking-tighter mb-1">
                                {scorecard.document_type ? `Review: ${scorecard.document_type}` : 'Investor Scorecard'}
                            </h2>
                            <p className={cn("text-xs font-bold uppercase tracking-widest", theme.textSubtle)}>AI-Driven Analysis</p>
                        </div>
                        <div className="h-16 w-16 rounded-2xl bg-black flex items-center justify-center text-white border border-white/20 shadow-xl shrink-0">
                            <span className="text-2xl font-black">{scorecard.total_score}</span>
                        </div>
                    </div>

                    <div className="space-y-6">
                        {categoriesToRender.map((cat: any, idx: number) => (
                            <div key={idx} className="space-y-2 group">
                                <div className="flex justify-between items-end">
                                    <span className="text-[10px] font-black uppercase tracking-wider">{cat.label}</span>
                                    <span className="text-sm font-black">{cat.score}/20</span>
                                </div>
                                ...
                                {isDynamic && scorecard.categories[idx]?.feedback && (
                                    <p className="text-xs text-gray-500 italic mt-1 leading-relaxed hidden group-hover:block transition-all">
                                        "{scorecard.categories[idx].feedback}"
                                    </p>
                                )}
                            </div>
                        ))}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className={cn("p-6 rounded-2xl border", theme.card, "bg-green-50/50 border-green-100")}>
                    ...
                    <ul className="space-y-3">
                        {scorecard.strengths?.length > 0 ? scorecard.strengths.slice(0, 3).map((s: string, i: number) => (
                            <li key={i} className="text-sm font-medium text-gray-700 flex items-start gap-2">
                                <span className={cn("mt-1.5 h-1.5 w-1.5 rounded-full bg-current shrink-0", s.toLowerCase() === 'no strengths' ? 'text-gray-400' : 'text-green-500')} />
                                {s}
                            </li>
                        )) : (
                            <li className="text-sm font-medium text-gray-500 italic">No strengths identified.</li>
                        )}
                    </ul>
                </div>
                <div className={cn("p-6 rounded-2xl border", theme.card, "bg-red-50/50 border-red-100")}>
                    ...
                    <ul className="space-y-3">
                        {scorecard.risks?.length > 0 ? scorecard.risks.slice(0, 3).map((r: string, i: number) => (
                            <li key={i} className="text-sm font-medium text-gray-700 flex items-start gap-2">
                                <span className={cn("mt-1.5 h-1.5 w-1.5 rounded-full bg-current shrink-0", r.toLowerCase() === 'no risks' ? 'text-gray-400' : 'text-red-500')} />
                                {r}
                            </li>
                        )) : (
                            <li className="text-sm font-medium text-gray-500 italic">No risks identified.</li>
                        )}
                    </ul>
                </div>
            </div>
...
export default function KasbReview() {
    const { user } = useAuth();
    ...
    useEffect(() => {
        ...
        // Reset current context when switching modes to ensure isolation
        setMessages([]);
        ...
        // Refresh history for the current mode
        if (user) {
            getUserChatSessions(user.id, mode === 'review' ? "kasb_review" : "kasb_studio").then((data) => {
                setSessions(data);
            });
        }
    }, [mode, user]);
    ...
    // Initial fetch of sessions handled by the mode-aware useEffect above.
    // This empty useEffect is kept as a placeholder if needed for other user-based initialization.
    useEffect(() => {
        // Initial load is now handled by mode useEffect
    }, [user]);
    ...
    const handleReview = async (file: File | null, userQuery: string) => {
        ...
        try {
            ...
            const userText = file 
                ? (userQuery.trim() ? `${userQuery}\n\n[Attached File: ${file.name}]` : `Analyze pitch deck: ${file.name}`)
                : userQuery.trim();

            if (!sessionId && user) {
                const session = await createChatSession(user.id, "Kasb Review", userText);
                if (session) {
            ...
            setMessages(prev => [...prev, { id: userMsgId, role: 'user', content: userText, created_at: new Date().toISOString(), session_id: sessionId || '' }]);
            
            if (sessionId) {
                await saveChatMessage(sessionId, 'user', userText);
            }

            const contentToReview = file || userQuery.trim();
            const result = await reviewDocumentDynamic(contentToReview, apiKey);
            ...
            if (sessionId) {
                await saveChatMessage(sessionId, 'assistant', finalContent);
            }
    ...
    const handleSendMessage = async () => {
        if (mode === 'review') {
            ...
            handleReview(selectedFile, query);
            return;
        }
        ...
        try {
            ...
            let sessionId = currentSessionId;
            if (!sessionId && user) {
                const personalityId = "Kasb Review";
                const session = await createChatSession(user.id, personalityId, userText);
                if (session) {
                    sessionId = session.id;
                    setCurrentSessionId(sessionId);
    ...
    const handleNewChat = () => {
        ...
        const list = REVIEW_PLACEHOLDERS;
