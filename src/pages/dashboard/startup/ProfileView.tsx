...
import { Sparkles, BarChart3, Info, TrendingUp, ShieldCheck, Pencil, Save, X, Loader2, FileText, ExternalLink, Globe, Package, History as HistoryIcon, Eye, EyeOff } from "lucide-react"
import { Link } from "react-router-dom"
import { QUESTIONNAIRE_CONFIG, DEFAULT_STAGE_CONFIG, isProfileComplete, getStartupMissingFields, calculateStartupProgress } from "../../../lib/questionnaire"
...
export function ProfileView({ startup, onRequestReview, onSave, saving, readOnly, id }: ProfileViewProps) {
    ...
    const _isComplete = isProfileComplete(startup)
    const missingFields = getStartupMissingFields(startup)
    ...
    const [isAutoFillEnabled, setIsAutoFillEnabled] = useState(() => localStorage.getItem('kasb_autofill_enabled') !== 'false')
    ...
    const handleToggleFeedVisibility = async () => {
        ...
        // If they are trying to turn it ON, enforce the 40% completion rule
        if (newValue) {
            const progress = calculateStartupProgress(startup)
            if (progress < 40) {
                toast("Complete your profile to show in feed", "error")
                return
            }
        }
    ...
    const handleToggleAutoFill = () => {
        const newValue = !isAutoFillEnabled
        setIsAutoFillEnabled(newValue)
        localStorage.setItem('kasb_autofill_enabled', newValue ? 'true' : 'false')
        toast(
            newValue ? "Auto-Fill Data Vault enabled." : "Auto-Fill Data Vault disabled.",
            "success"
        )
    }
            ...
            <Card className="border-0 shadow-none bg-transparent">
                <CardContent className="p-0 space-y-8">
                    ...
                    <div className={cn(
                        ...
                        {!isProfileComplete(startup) && !startup.show_in_feed && (
                    ...
                    {/* Auto-Fill Feature Toggle Card */}
                    {!readOnly && (
                        <div className={cn(
                            "mx-0 sm:mx-4 p-5 rounded-none sm:rounded-[2rem] border-x-0 sm:border transition-all duration-300 mt-4",
                            isAutoFillEnabled
                            ...
                            <div className="flex items-center justify-between gap-4">
                                <div className="flex items-center gap-4">
                                    <div className={cn(
                                        "h-11 w-11 rounded-2xl flex items-center justify-center shrink-0 transition-colors",
                                        isAutoFillEnabled ? "bg-indigo-100" : "bg-gray-100"
                                    )}>
                                        <Sparkles className={cn("h-5 w-5", isAutoFillEnabled ? "text-indigo-600" : "text-gray-400")} />
                                    </div>
                                    <div>
                                        <p className={cn(
                                            "text-[10px] font-black uppercase tracking-widest",
                                            isAutoFillEnabled ? "text-indigo-700" : "text-gray-500"
                                        )}>
                                            {isAutoFillEnabled ? "Auto-Fill Enabled" : "Auto-Fill Disabled"}
                                        </p>
                                        <p className={cn(
                                            "text-sm font-medium mt-0.5",
                                            isAutoFillEnabled ? "text-indigo-800" : "text-gray-600"
                                        )}>
                                            Shows a convenient copy-paste vault before visiting external VC websites.
                                        </p>
                                    </div>
                                </div>
                                ...
                                <button
                                    ...
                                    className={cn(
                                        "relative inline-flex h-7 w-12 shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none",
                                        isAutoFillEnabled ? "bg-indigo-500" : "bg-gray-300"
                                    )}
                                >
                                    <span className={cn(
                                        "pointer-events-none inline-block h-6 w-6 rounded-full bg-white shadow-lg transition-transform duration-200 ease-in-out",
                                        isAutoFillEnabled ? "translate-x-5" : "translate-x-0"
                                    )} />
                                </button>
                            </div>
                        </div>
                    ...
                    <div className="pt-4 px-4 sm:px-4">
                        {activeTab === 'questions' && (
                            <div className="space-y-10 animate-in fade-in slide-in-from-bottom-2 duration-300">
                                ...
                                {(startup.questionnaire?.product_sample?.url || startup.questionnaire?.product_sample?.image) && (
                                    <div className="pt-12 border-t border-gray-100 space-y-6">
                                        <div>
                                            <h3 className="text-xl font-extrabold tracking-tight">Product Showcase</h3>
                                            <p className="text-sm text-gray-500 mt-1">
                                                Explore the startup's product sample or MVP.
                                            </p>
                                        </div>

                                        <div className="bg-gray-50/50 rounded-[2rem] border border-gray-100 p-6 sm:p-8">
                                            {startup.questionnaire.product_sample.type === 'image' ? (
                                                <div className="space-y-4">
                                                    <div className="flex items-center gap-2 text-xs font-bold text-gray-400 uppercase tracking-widest mb-4">
                                                        <Package className="h-4 w-4" />
                                                        Product Prototype / Image
                                                    </div>
                                                    <div className="relative group rounded-3xl overflow-hidden border border-gray-200 bg-white shadow-sm">
                                                        <img 
                                                            src={startup.questionnaire.product_sample.image} 
                                                            alt="Product" 
                                                ...
                                                <div className="space-y-6">
                                                    ...
                                                    <div className="flex flex-col sm:flex-row items-center justify-between gap-6 bg-white p-6 rounded-3xl border border-gray-100 shadow-sm">
                                                        <div className="flex items-center gap-4">
                                                            ...
                                                            <div>
                                                                <p className="text-sm font-bold text-gray-900">Experience the Product</p>
                                                                <p className="text-xs text-gray-500 truncate max-w-[200px] sm:max-w-md">
                                                                    {startup.questionnaire.product_sample.url}
                                                                </p>
                                                            </div>
                                                        </div>
                                                        <Button
                                                            onClick={() => window.open(startup.questionnaire?.product_sample?.url, '_blank')}
                                                            className="w-full sm:w-auto bg-black text-white hover:bg-gray-800 rounded-full px-8 text-xs font-bold uppercase tracking-widest h-11"
                                                        >
                                                            Open Website
